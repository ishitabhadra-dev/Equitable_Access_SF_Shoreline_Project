...
# X positions for bars: horizontal labels (tickangle 0) with line breaks where needed.
CHART_X_LABELS: list[str] = [
    "Hispanic / Latino",
    "White",
    "Black",
    "American Indian<br>Alaska Native",
    "Asian",
    "Pacific Islander",
    "Some other race",
    "Multiracial",
]
# Hover and table export: same order as COMPOSITION_CATEGORIES, single-line names.
CHART_HOVER_LABELS: list[str] = [
    "Hispanic / Latino",
    "White",
    "Black",
...
def apply_presentation_theme(
    ...
    *,
    ...
    """
    High-contrast charts for presentations: dark #111827 text on white, horizontal
    category labels (no rotation), explicit font sizes, margins so nothing clips.
    """
    title_text = ""
    if fig.layout.title is not None and fig.layout.title.text is not None:
        title_text = fig.layout.title.text
    fig.update_layout(
        font=dict(size=14, color=CHART_FONT_COLOR, family="Arial, 'Helvetica Neue', sans-serif"),
        title=dict(
            text=title_text,
            x=0.02,
            xanchor="left",
            font=dict(size=20, color=CHART_FONT_COLOR, family="Arial, 'Helvetica Neue', sans-serif"),
        ),
        paper_bgcolor=CHART_PAPER,
        plot_bgcolor="#fafafa",
        hoverlabel=dict(
            font=dict(size=14, color=CHART_FONT_COLOR, family="Arial, sans-serif"),
            bgcolor="#ffffff",
            bordercolor="#94a3b8",
        ),
        margin=dict(l=48, r=48, t=72, b=bottom_margin),
        legend=dict(
            orientation="h",
            yanchor="top",
            y=legend_y,
            x=0.5,
            xanchor="center",
            font=dict(size=13, color=CHART_FONT_COLOR),
    ...
    fig.update_xaxes(
        tickangle=0,
        tickfont=dict(size=12, color=CHART_FONT_COLOR),
        title_font=dict(size=14, color=CHART_FONT_COLOR),
        automargin=True,
        showline=True,
        linewidth=1,
        linecolor="#9ca3af",
    ...
    fig.update_yaxes(
        ...
        title_font=dict(size=14, color=CHART_FONT_COLOR),
        showgrid=True,
        gridcolor="rgba(17, 24, 39, 0.09)",
        gridwidth=1,
        zeroline=True,
        zerolinewidth=1,
        zerolinecolor="rgba(17, 24, 39, 0.22)",
        showline=True,
        linewidth=1,
        linecolor="#9ca3af",
    )
...
# Table / CSV labels (single line, same order as charts).
AXIS_LABELS_SHORT: dict[str, str] = {
    cat: CHART_HOVER_LABELS[i] for i, cat in enumerate(COMPOSITION_CATEGORIES)
}
...
def fig_acs_vs_all_modes(
    ...
    *,
    ...
    x_chart = list(CHART_X_LABELS)
    hov = list(CHART_HOVER_LABELS)
    d_walk = by_mode["walk"]
    ...
    apply_presentation_theme(fig, bottom_margin=160, legend_y=-0.32)
    return fig
...
def fig_gap_all_modes(
    ...
    *,
    ...
    apply_presentation_theme(fig, bottom_margin=150, legend_y=-0.3)
    return fig
...
def fig_acs_vs_single_mode(
    ...
    *,
    ...
    apply_presentation_theme(fig, bottom_margin=130, legend_y=-0.2)
    return fig
...
def inject_page_styles() -> None:
    st.markdown(
        """
<style>
  /* Global readability (high contrast; Streamlit Plotly charts are in separate frames). */
  html, body, [class*="css"] {
...
def render_at_a_glance() -> None:
    """High-level scan: study one-liner + four light panels (no infographic stats)."""
...
<div style="max-width:960px;">
  ...
  <p class="section-intro">{pc.AT_A_GLANCE_INTRO}</p>
    ...
    st.markdown(pc.html_at_a_glance_grid(pc.AT_A_GLANCE_BLOCKS), unsafe_allow_html=True)
...
def county_evidence_callout(county: str, by_mode: dict[str, pd.DataFrame]) -> None:
    ...
    if gaps.empty or float(gaps.abs().max()) < 0.05:
        st.markdown(
            ...
            "Dark bars = whole county (ACS 2024). Colored bars = share of the modeled shoreline "
            "<strong>served</strong> population for walk, bike, or drive.</p>",
            unsafe_allow_html=True,
        )
        return
    idx = gaps.abs().idxmax()
    ...
    st.markdown(
        ...
        f"percentage points <strong>{direction}</strong> in the shoreline served mix under <strong>walk</strong> "
        f"than in the county as a whole <span style='color:#374151;'>({val:+.1f} pts).</span></p>",
        unsafe_allow_html=True,
    )
...
def render_key_findings() -> None:
    ...
    st.caption(
        "**What this means:** These bullets summarize where shoreline served mix lines up with, "
        "runs above, or runs below whole-county population share — by race/ethnicity and travel mode."
    )
