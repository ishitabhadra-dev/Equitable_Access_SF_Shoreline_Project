...
AT_A_GLANCE_BLOCKS: tuple[dict[str, str], ...] = (
    ...
    },
    {
        ...
        "stat_label": "",
        "body": (
            "<p style='margin:0 0 12px 0;color:#1f2937;font-size:1rem;line-height:1.55;'>"
            "Walk access frequently shows the largest deviation from county population share, "
            "though patterns vary by county.</p>"
            "<p style='margin:0 0 6px 0;font-weight:600;color:#111827;font-size:1rem;'>Examples</p>"
            "<ul style='margin:0;padding-left:1.2rem;color:#1f2937;font-size:1rem;line-height:1.65;'>"
            "<li>Santa Clara → largest drop in White access under walk</li>"
            "<li>Contra Costa → largest shifts across groups under walk</li>"
            "<li>San Mateo → walk shows strongest deviation for Asian population</li>"
            "</ul>"
        ),
    },
    {
        ...
        "body": (
            ...
            "</div><p style='margin:10px 0 0 0;color:#1f2937;font-size:1rem;line-height:1.55;'>"
    ...
    },
    {
        ...
        "stat_label": "ACS year",
        "body": (
            "Move from headline patterns to county examples and full charts below. "
            "Figures combine <strong>ACS 2024</strong> county shares with your "
            "<strong>shoreline access</strong> dataset — use them for regional discussion, "
            "not as a substitute for on-the-ground site review."
        ),
    },
)
...
QUICK_TAKEAWAYS_INTRO = (
    "Four patterns from this analysis. Use the county tabs and charts in the next sections "
    "to check the exact percentages for each county."
)
...
QUICK_TAKEAWAY_BLOCKS: tuple[dict[str, str], ...] = (
    ...
    },
    {
        ...
        "right_val": 2,
        "sentence": (
            "When access is measured as walking distance, some groups look much farther from their "
            "county-wide share than under bike or drive — mobility matters as much as nearness to water."
        ),
    },
    {
        ...
        "visual": "modes",
        "sentence": (
            "If who shows up in the served population changes when you switch modes, roads, paths, "
            "and transit — not only distance to the Bay — are shaping who can plausibly get there."
        ),
    },
)
...
def html_at_a_glance_grid(blocks: tuple[dict[str, str], ...]) -> str:
    ...
    for b in blocks:
        ...
        if b.get("stat"):
            stat_html = (
                f'<p class="ag-stat" style="font-size:2.1rem;font-weight:700;color:#111827;margin:0;line-height:1;">'
                f'{b["stat"]}'
                f'<span style="font-size:0.95rem;font-weight:600;color:#374151;margin-left:6px;">{b.get("stat_label", "")}</span></p>'
            )
        inner = stat_html + (
            f'<div style="margin-top:10px;color:#1f2937;font-size:1rem;line-height:1.55;">{b["body"]}</div>'
        )
...
<p style="margin:4px 0 0 0;font-size:0.85rem;color:#374151;text-align:center;">Walk · Bike · Drive</p>
        """
...
<div class="mode-sum-card" style="border-top:4px solid {c["color"]};">
  <p style="margin:0;font-size:0.78rem;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:#374151;">{c["mode"]}</p>
  <p style="margin:8px 0 6px 0;font-size:1.05rem;font-weight:700;color:#111827;">{c["headline"]}</p>
  <p style="margin:0;font-size:0.95rem;color:#1f2937;line-height:1.55;">{c["text"]}</p>
</div>
            """
        )
